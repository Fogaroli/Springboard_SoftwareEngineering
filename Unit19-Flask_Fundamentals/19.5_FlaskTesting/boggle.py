...
class Boggle:
    ...
    def make_board(self, columns, lines):
        ...
        for y in range(self.columns):
            row = [choice(string.ascii_uppercase) for i in range(self.lines)]
            board.append(row)
    ...
    def find_from(self, board, word, y, x, seen):
        ...
        if x > self.lines - 1 or y > self.lines - 1:
            return
        ...
        if y < self.columns - 1 and x < self.lines - 1:
            if self.find_from(board, word[1:], y + 1, x + 1, seen):
                return True
