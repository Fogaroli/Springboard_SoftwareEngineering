...
function vowelCount(str) {
    ...
    stringArray.forEach(function(letter) {
        if (/^[a,e,i,o,u]$/.test(letter.toLowerCase())) {
            if (letter.toLowerCase() in new_object) {
                new_object[letter.toLowerCase()] += 1;
            } else {
                new_object[letter.toLowerCase()] = 1;
            }
        }
...
function removeVowels(str) {
    return [...str]
        .filter(item => {
            return !/^[a,e,i,o,u]$/.test(item.toLowerCase());
        })
        .join("")
        .toLowerCase();
}
...
function doubleOddNumbers(arr) {
    const odd_numbers = arr.filter(odd => {
        return odd % 2 !== 0;
    });
    return odd_numbers.map(item => {
        return item * 2;
    });
}
