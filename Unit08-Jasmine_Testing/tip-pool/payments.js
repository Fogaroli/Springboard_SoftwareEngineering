...
function appendPaymentTable(curPayment) {
  ...
  appendDeleteBtn(newTr, removePayment);
...
// remove a server form the server table and updates the table with new servers
function removePayment(evt){
  let removeTarget = evt.target.parentElement.parentElement.id;
  delete allPayments[removeTarget];
